...
const setupSteps = [
  ...
  },
  {
    ...
    description: 'Assign each button to the scene you want to trigger (for example: 1 = Starting Soon, 2 = Live Main).',
...
const setupCards = setupSteps
  .map(
    (step, index) => `
      <article class="card" aria-label="${step.title}">
        <div class="step-tag">${step.subtitle}</div>
        <h3>${index + 1}. ${step.title}</h3>
        <p>${step.description}</p>
        <div class="file-row">
          <code>${step.file}</code>
          <button class="copy-btn" type="button" data-copy="${step.file}">Copy path</button>
        </div>
      </article>
    `
  )
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      ...
      window.setTimeout(() => {
        ...
        button.textContent = previousText ?? 'Copy path';
      }, 1300);
    } catch (error) {
...
console.log('Guided web UI loaded: creator-focused setup experience ready.');
